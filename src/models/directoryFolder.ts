...
function makeCutout(
  ...
  if (shape === 'circle' || shape === 'disk' || shape === 'dot') {
    ...
    const backing = new THREE.Mesh(
      new THREE.CylinderGeometry(radius + 0.055, radius + 0.055, 0.045, sides),
      new THREE.MeshStandardMaterial({ color: '#FFFDF5', roughness: 0.92 }),
    );
    const artwork = new THREE.Mesh(
      new THREE.CylinderGeometry(radius, radius, 0.035, sides),
  ...
  return {
    backing: makeExtrudedMesh(roundedRectShape(0.62, height + 0.08, 0.09), '#FFFDF5', 0.045, 0.018),
    artwork: makeExtrudedMesh(roundedRectShape(0.53, height - 0.02, 0.065), color, 0.035, 0.012),
  };
}
...
function addCollage(group: THREE.Group, parts: Map<string, THREE.Object3D>, variant: CollageVariant): void {
  ...
  placements.forEach((placement, index) => {
    ...
    cutout.backing.position.set(placement.x, placement.y, index * 0.018);
    cutout.artwork.position.set(placement.x, placement.y, 0.055 + index * 0.018);
