...
describe('项目经历页面', () => {
  ...
  it('prints one reading button per source beside the fifth card', () => {
    const card = projectShowcases[4]!;
    const markup = buildProjectsPageMarkup([card]);

    expect(card.links).toHaveLength(5);
    expect(markup.match(/data-project-link="/g)).toHaveLength(5);
    expect(markup.match(/data-project-link-index="/g)).toHaveLength(5);
    for (const link of card.links!) {
      // Short label on the pill, full headline in the tooltip and the
      // accessible name, and the source itself behind it: four WeChat pieces
      // and the planning deck that ships with the site.
      expect(markup).toContain(`>${link.label}</span>`);
      expect(markup).toContain(`title="${link.headline}"`);
      expect(markup).toContain(link.action ?? '阅读原文');
    }
    // The stills keep their own buttons; the reading list is additive.
    expect(markup.match(/data-project-zoom=/g)).toHaveLength(8);
  });
